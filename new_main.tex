...
test set. Because macro and weighted F1 can rank models very differently under
severe class imbalance, we report both throughout, following the guidance on
evaluation-measure choice for imbalanced classification tasks given by
Sokolova and Lapalme~\cite{sokolova2009systematic} and by Saito and
...
\begin{itemize}
  ...
  \item \textbf{Hyperparameters:} As described in
        Section~\ref{sec:model_construction}.
  \item \textbf{Evaluation measures:} Test-set Accuracy, Macro Precision,
    ...
    detection); weighted averaging reflects real-world class frequency. This
    macro/weighted distinction, and its importance under class imbalance, is
...
\begin{itemize}
  ...
  \item Deep learning models (LSTM, BiLSTM) achieve the highest accuracy and
        are readily extendable to pre-trained embeddings such as
        word2vec~\cite{mikolov2013efficient} or GloVe~\cite{pennington2014glove},
        or to pre-trained clinical language models such as
        ClinicalBERT~\cite{alsentzer2019publicly}.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Residual leakage.} 181 reports retain partial BI-RADS cue
        words post-cleaning.
  \item \textbf{No transformer models.} Models such as
        ClinicalBERT~\cite{alsentzer2019publicly} were not evaluated in this
        study.
  \item \textbf{Single language pair.} The paradigm has been validated only for
        Portuguese$\to$English; the broader translate-test literature (e.g.
        \cite{artetxe2023revisiting,papaioannou2022cross}) suggests results
        may not transfer directly to other language pairs.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Transformer fine-tuning.} Fine-tuning ClinicalBERT~\cite{alsentzer2019publicly},
        BioBERT, or a Portuguese clinical BERT directly on original Portuguese text.
  \item \textbf{Multilingual models.} Applying mBERT or XLM-R to raw
        Portuguese reports to eliminate translation noise, in the spirit of
        the cross-lingual transfer strategies studied by Papaioannou et
        al.~\cite{papaioannou2022cross} and the translate-test analysis of
        Artetxe et al.~\cite{artetxe2023revisiting}.
  \item \textbf{Class imbalance mitigation.} Oversampling with
        SMOTE~\cite{chawla2002smote}, focal loss~\cite{lin2017focal},
  ...
  \item \textbf{Multi-modal integration.} Combining report text with
        ...
        mammography models such as~\cite{yala2019deep}, in a joint model.
\end{itemize}
