...
\multicolumn{7}{l}{\footnotesize $^\ast$Nonlinear SVM (RBF kernel) metrics reported on validation set.} 
\end{tabular} 
\end{table}
...
We also evaluated a nonlinear SVM with an RBF kernel, but it underperformed
both the linear SVM and Logistic Regression across key metrics (macro F1 of
0.627 with TF-IDF, and 0.596 with CountVectorizer). This drop suggests that
projecting the high-dimensional textual feature space into a non-linear,
infinite-dimensional space offers no discriminative advantage for this corpus,
and instead causes the model to overfit or struggle with the extreme
class imbalance compared to the simpler linear hyperplane.
