...
and its inherently discriminative objective. The linear SVM~\cite{joachims1998text}
does best on weighted F1 (0.900), largely because the dominant BI-RADS~2 class is easy to
separate in TF-IDF space and weighted F1 rewards getting the majority class
right. Naive Bayes~\cite{mccallum1998naive} struggles under TF-IDF---its independence assumption does
not play well with dense, continuous TF-IDF weights---and Random
Forest~\cite{breiman2001random}, despite
balanced class weights, still under-serves the minority classes, since
tree-based splits tend to favour whichever class dominates a node. XGBoost~\cite{chen2016xgboost}
tracks a similar pattern to Random Forest on macro metrics despite its
stronger boosting objective, again reflecting how little signal a handful of
Category-5/6 examples can provide to a tree ensemble.
...
warrants a closer look at how that performance is distributed across BI-RADS
categories and how confident its predictions are.
Figure~\ref{fig:lstm_per_class_f1} shows the per-class F1 scores for all
three recurrent architectures.
The LSTM achieves near-perfect F1 for Category~2 (0.97), which is expected
given that over 80\% of the test set belongs to this class.
For Categories~0 and~1, the LSTM reaches 0.77 and 0.85 F1 respectively,
outsperforming the RNN on both while remaining slightly behind the BiLSTM on
Category~0.
The most clinically significant gap is at Categories~3 and~4: the LSTM reaches
F1 scores of 0.54 and 0.59 respectively, substantially better than the plain
RNN (0.41 and~0.00) and comparable to the BiLSTM, showing that gated recurrence
is essential for capturing the nuanced language distinguishing
``probably benign'' from ``suspicious'' findings.
Beyond per-class F1, the macro-averaged ROC analysis
(Figure~\ref{fig:lstm_roc}) reveals that the LSTM achieves an overall AUC of
0.971, the highest among the three architectures (BiLSTM: 0.966, RNN: 0.949),
indicating that the LSTM's posterior probability scores discriminate between
BI-RADS categories well even for classes where hard predictions fail.
Finally, the prediction confidence analysis
(Figure~\ref{fig:lstm_confidence}) shows a clear separation between the
LSTM's correct and incorrect predictions: correctly classified reports are
assigned maximum softmax probabilities tightly clustered near 1.0
(interquartile range 0.97--1.0), whereas incorrectly classified reports have
a much wider spread (IQR approximately 0.58--0.89).
This gap---absent in the RNN but partially present in the BiLSTM---suggests
that for the LSTM, a low maximum class probability can serve as a practical
confidence flag to route uncertain reports for radiologist review.
Explainability analysis using SHAP (Shapley Additive exPlanations, using
\texttt{shap.Explainer} with a text masker) was applied to three representative
test reports, producing token-level attributions for each BI-RADS class
probability. The resulting SHAP text plots confirm that the LSTM attends most
strongly to measurement expressions (e.g., ``2.5 cm'', ``3$\times$2 cm''),
negation tokens (``no mass'', ``without calcification''), and laterality terms
(``right breast'', ``bilateral'')---precisely the tokens preserved by the
clinically-aware preprocessing pipeline---while treating generic connector
words as near-zero contributors. LIME (Local Interpretable Model-agnostic
Explanations) analysis, applied with 15 top influential words per instance,
corroborated these findings, with terms such as ``nodule'', ``irregular'',
and ``calcification'' driving predictions toward higher-risk BI-RADS
categories and words like ``no'', ``normal'', and ``benign finding'' anchoring
predictions at Category~2.
...
The pipeline is modular and reproducible, with deterministic tokenisation,
vocabulary construction, data splits, and evaluation throughout, and it
compares three model families under a single, unified protocol rather than
evaluating them in isolation. The N-gram models bring the added benefit of
being human-inspectable: their class language statistics and calibrated MAP
scores can be examined directly, which is not something the other two
families offer as naturally. Among the classical models, Logistic Regression
with balanced class weights offers the best macro-averaged F1 without
requiring any GPU infrastructure, making it an attractive option where
compute is limited. The deep learning models, LSTM and BiLSTM in particular,
achieve the highest raw accuracy of any family tested, and are readily
extendable to pre-trained embeddings such as word2vec~\cite{mikolov2013efficient}
or GloVe~\cite{pennington2014glove}, or to pre-trained clinical language
models such as ClinicalBERT~\cite{alsentzer2019publicly}.

\subsection{Limitations}

Several limitations temper these results. Class imbalance remains severe:
Categories~5 and~6 have only 20 and 32 training samples respectively, which
causes all models to achieve zero or near-zero F1 on these classes.
Translation quality is another open question, since machine-translated text
may introduce artefacts and quality was assessed only by a spot-check on 50
reports rather than a systematic audit. A degree of residual leakage also
remains, with 181 reports retaining partial BI-RADS cue words even after
cleaning. No transformer-based models were evaluated in this study, so
models such as ClinicalBERT~\cite{alsentzer2019publicly} are absent from the
comparison. Finally, the translate-then-classify paradigm has been validated
only for the Portuguese$\to$English pair used here; the broader
translate-test literature (e.g.\ \cite{artetxe2023revisiting,papaioannou2022cross})
suggests these results may not transfer directly to other language pairs.
